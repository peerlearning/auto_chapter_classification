...
# Removing non-UTF characters for word clouds
```{r eval=FALSE}
...
```{r}
repo$chapter <- factor(repo$chapter)
str(repo$chapter)
```
...
```{r}
text_train_labels <- repo[-test_indices,  ]$chapter
text_test_labels <- repo[test_indices,  ]$chapter 
...
# wordcloud(text_corpus_clean[repo$Chapter == "Fundamentals of Mathematics"], max.words = 40, scale = c(3, 0.5))
# wordcloud(text_corpus_clean[repo$Chapter == "Conic Sections - I"], max.words = 40, scale = c(3, 0.5))
```
...
## Converting DTM to a categorical data (T/F)
